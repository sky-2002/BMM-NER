...
def get_dataloaders(file_paths, sample_size=None):
...
def get_test_dataloader(file_paths, batch_size=8, sample_size=None):
    with open("./le.pkl", "rb") as f:
       le = pickle.load(f)
    sdf = df_for_ner(file_paths=file_paths, le=le)
    if sample_size:
    ...
    test_dataset = Dataset(texts= test_sent, tags= test_tags)
